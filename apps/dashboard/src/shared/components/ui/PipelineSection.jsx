...
import { FiRefreshCw, FiFilter, FiTrendingUp, FiPlus, FiSearch, FiAlertCircle, FiUpload, FiTrash2, FiMinimize2 } from 'react-icons/fi';
...
const PipelineSection = ({ isAdmin = false }) => {
  const {
    ...
    manualRefresh, addLead, updateLead, moveLead, refreshData, addTag, removeLead,
  } = usePipeline();
  ...
  const handleManualRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      await (refreshData?.() || Promise.resolve());
      toast.success('Pipeline data refreshed');
    } catch (e) {
      toast.error(String(e?.message || 'Failed to refresh data'));
    } finally {
      setRefreshing(false);
    }
  }, [refreshData]);
  ...
  const submitNewLead = useCallback(async ({ contact, transaction, primaryTag }) => {
    ...
    try {
      ...
      if (msg.includes('403') || msg.toLowerCase().includes('does not have access to this location')) {
        ...
        toast.error('Contact saved, but adding lead to pipeline failed.');
      }
      throw e; // surface so modal can show error if desired
    }
  }, [addLead, addTag, stages]);

  const handleUpdateLead = useCallback(async (leadId, updates) => {
    // provided by context
    try { await updateLead(leadId, updates); toast.success('Lead updated'); }
    catch { toast.error('Failed to update lead'); }
  }, [updateLead]);

  const handleMoveLead = useCallback(async (leadId, fromStage, toStage) => {
    try { await moveLead(leadId, fromStage, toStage); toast.success('Lead moved'); }
