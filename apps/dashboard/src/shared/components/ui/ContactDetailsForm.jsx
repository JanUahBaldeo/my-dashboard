...
const ContactDetailsForm = memo(function ContactDetailsForm({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        else if (!cancelled) toast.error('Failed to load users');
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) toast.error(`Failed to load contact${e?.message ? `: ${e.message}` : ''}`);
      } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`Failed to save contact${e?.message ? `: ${e.message}` : ''}`);
    } finally {
      setSaving(false);
    }
