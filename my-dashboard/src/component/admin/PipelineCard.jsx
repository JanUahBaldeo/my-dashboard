...
const PipelineCard = ({ name, address, loanType, closeDate, status, loanAmount }) => {
  const badgeStyles = {
  ...
  const renderField = (label, value, field, isNumeric = false, tooltip = '') => {
    useEffect(() => {
    ...
    return (
      <div className="text-xs mt-2 group relative" title={tooltip}>
        ...
        {editingField === field ? (
          ...
          <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="text-gray-500 group-hover:underline cursor-help">
  ...
  return (
    <motion.div layout initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }} className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm text-sm">
